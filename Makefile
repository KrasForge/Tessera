...
# Functional instruction-count model, not Cortex-A72 hardware time.
SESSION_QEMU_FLAGS ?= -accel tcg,thread=single -icount shift=0,align=off,sleep=off
SESSION_CONSOLE_MODE ?= deterministic
SESSION_OUTPUT ?= $(ARM_BUILD_DIR)/session
SESSION_DEFS ?=
...
build-arm-session: $(ARM_BUILD_DIR)/plugin_session_source.elf $(ARM_BUILD_DIR)/plugin_session_gain.elf \
    $(ARM_BUILD_DIR)/plugin_hog.elf $(ARM_BUILD_DIR)/plugin_good.elf $(ARM_BUILD_DIR)/plugin_effect_filter.elf
	mkdir -p $(SESSION_OUTPUT)
	set -e; for s in $(SESSION_SRCS); do \
	    $(ARM_CC) $(ARM_CFLAGS) $(VIRT_MMU_FLAGS) $(VIRT_GIC_FLAGS) -c $$s -o $(SESSION_OUTPUT)/$$(basename $${s%.c}).o; \
	done
	set -e; for s in $(VIRT_DIR)/start_virt.S $(ARCH_ARM_DIR)/smp_entry.S $(ARCH_ARM_DIR)/vectors.S \
	    ...
	    $(ARM_CC) $(ARM_ASFLAGS) -I$(ARCH_ARM_DIR) -c $$s -o $(SESSION_OUTPUT)/$$(basename $${s%.S}).o; \
	done
	$(ARM_CC) $(ARM_CFLAGS) $(VIRT_MMU_FLAGS) $(VIRT_GIC_FLAGS) -c $(VIRT_DIR)/uart_virt.c -o $(SESSION_OUTPUT)/uart.o
	$(ARM_CC) -I$(ARCH_ARM_DIR) $(ARM_CFLAGS) $(VIRT_MMU_FLAGS) $(VIRT_GIC_FLAGS) $(SESSION_DEFS) -c $(VIRT_DIR)/session_main.c -o $(SESSION_OUTPUT)/main.o
	$(ARM_LD) -T $(VIRT_DIR)/session.ld -o $(SESSION_OUTPUT)/kernel.elf $(SESSION_OUTPUT)/*.o

test-arm-session-qemu:
	$(MAKE) -j1 build-arm-session SESSION_OUTPUT=$(ARM_BUILD_DIR)/session-capacity SESSION_DEFS="-DSESSION_AUTOTEST -DSESSION_FRAMES=240"
	timeout 45 qemu-system-aarch64 -machine virt -cpu cortex-a72 -smp 4 -m 256M \
	    -display none -serial file:$(ARM_BUILD_DIR)/session-capacity/acceptance.log -net none \
	    -kernel $(ARM_BUILD_DIR)/session-capacity/kernel.elf $(SESSION_QEMU_FLAGS)
	@cat $(ARM_BUILD_DIR)/session-capacity/acceptance.log
	@grep -q '^SESSION: PASS' $(ARM_BUILD_DIR)/session-capacity/acceptance.log

run-arm-workstation:
	$(MAKE) -j1 build-arm-session SESSION_DEFS=
	python3 scripts/run_workstation.py --build-dir $(ARM_BUILD_DIR)

.PHONY: test-arm-session-console
test-arm-session-console:
	$(MAKE) -j1 build-arm-session SESSION_DEFS=
	python3 scripts/test_session_console.py --build-dir $(ARM_BUILD_DIR) --mode $(SESSION_CONSOLE_MODE)
...
.PHONY: test-arm-m11 test-arm-m13 test-arm-session-tsan
test-arm-m11:
	$(MAKE) -j1 test-arm-temporal-multicore test-arm-session-concurrency test-arm-session-qemu

test-arm-m13:
	$(MAKE) -j1 test-arm-session-codec test-arm-shell test-arm-shell-graph test-arm-patch test-arm-session-console

test-arm-session-tsan: | $(ARM_BUILD_DIR)
	$(CC) -std=c11 -Wall -Wextra -g -O1 -fsanitize=thread \
	    -DHOSTTEST -I$(ARCH_ARM_DIR) -Iinclude $(SESSION_HOST_SRCS) -pthread \
	    -o $(ARM_BUILD_DIR)/session_concurrency_tsan
	TSAN_OPTIONS=halt_on_error=1 timeout 60 $(ARM_BUILD_DIR)/session_concurrency_tsan
