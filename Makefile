...
# Host unit tests for partitioned FFT convolution (Theme M18, issue #185):
# streaming output vs a brute-force reference and the direct tessera_conv
# engine across 1..8-partition IRs, delta identity/delay, reset.  ASan/UBSan.
ARM_PCONV_TEST_SRCS = tests/arm64/pconv_test.c sdk/lib/tessera_pconv.c \
                      sdk/lib/tessera_fft.c sdk/lib/tessera_conv.c
test-arm-pconv: | $(ARM_BUILD_DIR)
	$(CC) -std=c11 -Wall -Wextra -g -O1 -fsanitize=address,undefined \
	      -Isdk $(ARM_PCONV_TEST_SRCS) -o $(ARM_BUILD_DIR)/pconv_test -lm
	$(ARM_BUILD_DIR)/pconv_test

# Host unit tests for the phase vocoder (Theme M18, issue #186): unity
# reproduces the input, 2x stretch preserves pitch, +/-12 st shifts move the
# measured fundamental an octave (Goertzel), duration preserved.  ASan/UBSan.
ARM_PVOC_TEST_SRCS = tests/arm64/vocoder_test.c sdk/lib/tessera_pvoc.c \
                     sdk/lib/tessera_fft.c sdk/lib/tessera_math.c
test-arm-vocoder: | $(ARM_BUILD_DIR)
	$(CC) -std=c11 -Wall -Wextra -g -O1 -fsanitize=address,undefined \
...
# Host unit tests for the modulation matrix (Theme M19, issue #188): a real
# control-rate LFO sweeps its destination between the expected bounds, multiple
# sources sum with independent depths and clamp, depth-0/removed routes leave
# the base, curves shape the source.  ASan/UBSan.
ARM_MOD_TEST_SRCS = tests/arm64/modmatrix_test.c sdk/lib/tessera_mod.c \
                    sdk/lib/tessera_dsp.c sdk/lib/tessera_math.c
test-arm-modmatrix: | $(ARM_BUILD_DIR)
	$(CC) -std=c11 -Wall -Wextra -g -O1 -fsanitize=address,undefined \
...
test-arm-multiio: | $(ARM_BUILD_DIR)
	$(CC) -std=c11 -Wall -Wextra -g -O1 -fsanitize=address,undefined \
...
test-arm-multicore-legacy-qemu: $(ARM_BUILD_DIR)/plugin_good.elf \
                         $(ARM_BUILD_DIR)/plugin_crash.elf \
                         $(ARM_BUILD_DIR)/plugin_evil2.elf \
                         $(ARM_BUILD_DIR)/plugin_hog.elf
...
test-arm-shell-qemu: | $(ARM_BUILD_DIR)
	...
	printf 'help\recho hello world\rstat\recho hel\bllo\rbad\rfrobnicate\rdone\r' \
	  | timeout 30 qemu-system-aarch64 -machine virt -cpu cortex-a72 -smp 4 -m 256M \
	    ...
	    -kernel $(VIRT_SHELL_ELF) > $(ARM_BUILD_DIR)/virt_shell.log 2>&1
...
test-arm-shell-graph-qemu: $(ARM_BUILD_DIR)/plugin_good.elf \
	...
	printf 'ls\rload /rd/synth\rload /rd/effect\rwire 1 2\rwire 2 dac\rset-param 1 0 660\rls\rrun 8\rstats\rdone\r' \
	  | timeout 30 qemu-system-aarch64 -machine virt -cpu cortex-a72 -smp 1 -m 256M \
	    -display none -serial stdio -monitor none -net none \
	    -kernel $(VIRT_SHGRAPH_ELF) > $(ARM_BUILD_DIR)/virt_shell_graph.log 2>&1
...
test-arm-shell-patch-qemu: sdk-example $(ARM_BUILD_DIR)/plugin_effect_filter.elf
	...
	printf 'load /rd/synth\rload /rd/effect\rwire 1 2\rwire 2 dac\rset-param 1 0 880\rrun\rpatch save /sd/live.patch\rreboot\rpatch ls\rpatch load /sd/live.patch\rrun\rpatch load /sd/missing.patch\rdone\r' \
	  | timeout 30 qemu-system-aarch64 -machine virt -cpu cortex-a72 -smp 1 -m 256M \
	    -display none -serial stdio -monitor none -net none \
	    -kernel $(VIRT_SHPATCH_ELF) > $(ARM_BUILD_DIR)/virt_shell_patch.log 2>&1
...
build-arm-session: $(ARM_BUILD_DIR)/plugin_session_source.elf $(ARM_BUILD_DIR)/plugin_session_gain.elf \
    ...
    $(ARM_BUILD_DIR)/plugin_session_crash.elf $(ARM_BUILD_DIR)/plugin_effect_filter.elf
...
test-arm-session-console:
	# M13 functional acceptance: 12 kHz / 240 samples = 20 ms. The shell,
	# persistence and exact-PCM assertions are unchanged; hardware timing is M10.
	$(MAKE) -j1 build-arm-session SESSION_DEFS="-DSESSION_FRAMES=240 -DSESSION_RATE=12000"
...
# Complete M13 acceptance. The legacy issue fixtures prove the original UART,
# synth->filter graph and console patch requirements; the production workstation
# adds live multicore edits and a true second QEMU boot with persisted FAT state.
.PHONY: test-arm-m13
test-arm-m13:
	$(MAKE) -j1 arm test-arm-shell test-arm-shell-graph test-arm-patch \
	    test-arm-session-codec test-arm-session-concurrency \
	    test-arm-shell-qemu test-arm-shell-graph-qemu test-arm-shell-patch-qemu \
	    test-arm-session-console


# Complete integrated acceptance. Keep execution serial: M11 and M13 share
# plugin/session build outputs, and both dedicated gates exercise QEMU SMP.
.PHONY: test-arm-m11-m13
test-arm-m11-m13:
	$(MAKE) -j1 test-arm-m11 test-arm-m13

# Strict M11 acceptance: actual same EL0 workload overloaded on CPU1, then
# admitted on CPU1..3; real low-pass bit-equivalence and all-core faults.
$(ARM_BUILD_DIR)/plugin_session_crash.elf: plugins/test/session_probe.c $(PLUGIN_LD) | $(ARM_BUILD_DIR)
	$(ARM_CC) $(PLUGIN_CFLAGS) -DPROBE_CRASH=1 -Iinclude -c $< -o $(ARM_BUILD_DIR)/session_crash.o
	$(ARM_LD) -T $(PLUGIN_LD) -o $@ $(ARM_BUILD_DIR)/session_crash.o

M11_REPEAT ?= 3
# QEMU MTTCG acceptance profile: 12 kHz / 240 samples = 20 ms.  The workload
# is 40% of a frame per plugin, so three identical jobs exceed one core while
# each fits comfortably on its own worker despite host scheduling jitter.
# This is software-scheduling evidence, not a CM4 latency/WCET claim.
M11_RATE ?= 12000
M11_FRAMES ?= 240
.PHONY: build-arm-m11 test-arm-multicore-qemu test-arm-m11 test-arm-m11-live-qemu
build-arm-m11:
	$(MAKE) -j1 build-arm-session SESSION_DEFS="-DSESSION_AUTOTEST -DSESSION_M11 -DSESSION_FRAMES=$(M11_FRAMES) -DSESSION_RATE=$(M11_RATE)"
	mkdir -p $(ARM_BUILD_DIR)/m11
	cp $(ARM_BUILD_DIR)/session/kernel.elf $(ARM_BUILD_DIR)/m11/kernel.elf

test-arm-multicore-qemu: build-arm-m11
	python3 scripts/run_qemu_timing.py --fixture multicore --mode wallclock --repeat $(M11_REPEAT) --build-dir $(ARM_BUILD_DIR)

test-arm-m11:
	$(MAKE) -j1 arm test-arm-worker test-arm-worker-pause test-arm-gsched \
	    test-arm-temporal-multicore test-arm-session-concurrency \
	    test-arm-m11-fault-record test-arm-multicore-qemu test-arm-m11-live-qemu

# Original issue-level live QEMU acceptance for #74/#75.  #76 is exercised
# above by the shared-session multicore fixture, using the same runtime that
# M13 controls.  Keeping these as an explicit aggregate also makes `test-arm-m11`
# self-contained instead of referring to a missing target.
test-arm-m11-live-qemu: test-arm-worker-qemu test-arm-gsched-qemu

.PHONY: test-arm-m11-fault-record
test-arm-m11-fault-record: | $(ARM_BUILD_DIR)
	$(CC) -std=c11 -Wall -Wextra -Werror -g -O1 -fsanitize=address,undefined \
	    -I$(ARCH_ARM_DIR) tests/arm64/process_fault_test.c -pthread \
	    -o $(ARM_BUILD_DIR)/m11_fault_record_test
	$(ARM_BUILD_DIR)/m11_fault_record_test
