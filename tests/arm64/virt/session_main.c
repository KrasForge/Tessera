...
extern char session_hog_start[], session_hog_end[], session_sine_start[],
    session_sine_end[];
extern char session_filter_start[], session_filter_end[];
#define RATE 48000u
...
static void storage(void) {
  ...
  if (fresh_disk) {
    ...
    u16(session_sd + 22, 32);
    session_sd[0] = 0xeb;
    session_sd[1] = 0x3c;
    session_sd[2] = 0x90;
    const char *oem = "TESSERA ";
    for (unsigned i = 0; i < 8; ++i)
      session_sd[3 + i] = (uint8_t)oem[i];
    session_sd[21] = 0xf8;
    session_sd[36] = 0x80;
    session_sd[38] = 0x29;
    const char *label = "TESSERA    ";
    for (unsigned i = 0; i < 11; ++i)
      session_sd[43 + i] = (uint8_t)label[i];
    const char *kind = "FAT16   ";
    for (unsigned i = 0; i < 8; ++i)
      session_sd[54 + i] = (uint8_t)kind[i];
    u16(session_sd + 512, 0xfff8);
    u16(session_sd + 514, 0xffff);
    session_sd[510] = 0x55;
    session_sd[511] = 0xaa;
  }
  ...
  if (fresh_disk) {
    ...
    require(
        ...
        "synth file");
    require(fat_write_file(
                &filesystem, "FILTER.ELF", (uint8_t *)session_filter_start,
                (uint32_t)(session_filter_end - session_filter_start)) > 0,
            "filter file");
  }
...
static int inspect_cmd(shell_t *sh, int n, char **v) {
  ...
  uart_printf("scheduler: accepted=%u skipped=%u invalid=%u last=%d gate=%u "
              "frame=%u callback=%u\r\n",
              (unsigned)app.runtime.accepted, (unsigned)app.runtime.skipped,
              (unsigned)app.invalid_kicks, app.last_kick_result,
              app.runtime.gate, (unsigned)app.runtime.frame,
              (unsigned)sequence);
  for (unsigned i = 0; i < 3; ++i)
    uart_printf("worker%u: sent=%u done=%u blocks=%u\r\n", i + 1,
                (unsigned)workers[i].block_seq, (unsigned)workers[i].done_seq,
                (unsigned)workers[i].blocks);
  return 0;
}
...
static void automatic(void) {
   ...
   * Reserve 70% per core including cold EL0/IRQ entry in MTTCG; this is
   * an explicit test allowance, not a measured hardware WCET. */
  temporal_contract_t heavy = contract(ticks * 7 / 10),
                      light = contract(ticks / 8);
  uint32_t pids[3];
  for (unsigned i = 0; i < 3; ++i) {
  ...
  audio_wd_t serial_watchdog;
  audio_wd_init(&serial_watchdog, ticks);
  audio_wd_account(&serial_watchdog, serial);
  require(serial > ticks && serial_watchdog.overruns == 1,
          "serial measured deadline violation");
  ...
  require(!session_wire(&app, (uint32_t)source, (uint32_t)gain, 0) &&
          ...
          "chain wiring");
  require(!session_set_cores(&app, 1), "serial reference placement");
  wrong_samples = 0;
  verify_samples = 1;
  require(!session_start(&app), "serial reference start");
  frames(33);
  require(!session_pause(&app) && !wrong_samples, "serial reference PCM");
  uint64_t serial_hash = __atomic_load_n(&hash, __ATOMIC_RELAXED);
  require(!session_set_cores(&app, 3), "restore multicore capacity");
  ...
  for (unsigned cpu = 1; cpu <= 3; ++cpu) {
    ...
    long crash = session_load(&app, "/sd/SOURCE.ELF", &light);
    require(crash > 0 && !session_pin(&app, (uint32_t)crash, cpu),
            "crasher placement");
    require(!session_set_param(&app, (uint32_t)crash, 2, 0x3f800000),
            "arm memory fault");
    wrong_samples = 0;
    verify_samples = 1;
    ...
    uart_printf("hog cpu=%u pid=%u snapshot=%d runs=%u complete=%u killed=%u "
                "budget=%u deadline=%u faults=%u streak=%u\r\n",
                cpu, (unsigned)hog, got_state, (unsigned)st.runs,
                (unsigned)st.completed, st.killed, (unsigned)st.budget_overruns,
                (unsigned)st.deadline_misses, (unsigned)st.faults, st.streak);
    require(!got_state && st.killed && st.budget_overruns == 3,
            "hog terminated on each core");
    require(!wrong_samples && !app.runtime.skipped,
            "fault did not corrupt chain");
    tc_task_state_t dead;
    require(!tm_snapshot(&app.runtime, (uint32_t)crash, &dead) && dead.killed &&
                dead.faults == 1,
            "memory fault isolated on each core");
    require(__atomic_load_n(&hash, __ATOMIC_RELAXED) == serial_hash,
            "parallel samples equal serial samples");
    require(!session_unload(&app, (uint32_t)crash), "crasher cleanup");
  ...
  require(!irq_overruns, "CPU0 callback budget");
