...
extern char session_hog_start[], session_hog_end[], session_sine_start[],
    session_sine_end[], session_crash_start[], session_crash_end[],
    session_filter_start[], session_filter_end[];
#ifndef SESSION_RATE
...
static uint32_t quit_requested, verify_samples;
#ifndef SESSION_AUTOTEST
static uint32_t quitting;
#endif
...
#ifdef SESSION_M11
/* Test-only unadmitted negative control; the admitted runtime is paused. */
static uint32_t serial_mode;
static uint64_t window_end, cpu0_svcs, plugin_syscalls;
static unsigned capture_mode, captured;
static int16_t reference_pcm[64][FRAMES * 2];
void syscall_trace(uint64_t num) {
  if (!window_end) return;
  if (el0_cpu_index() == 0) ++cpu0_svcs;
  if (num != SYS_EXIT) __atomic_fetch_add(&plugin_syscalls, 1u, __ATOMIC_RELAXED);
}
#endif
...
static void storage(void) {
  ...
  if (fresh_disk) {
    require(fat_write_file(&filesystem, "CRASH.ELF", (uint8_t *)session_crash_start,
                           (uint32_t)(session_crash_end - session_crash_start)) > 0,
            "crash file");
    require(fat_write_file(&filesystem, "FILTER.ELF", (uint8_t *)session_filter_start,
...
#ifdef SESSION_M11
  if (serial_mode) {
    /* CPU0 keeps servicing callbacks; an overdue worker is never awaited. */
    (void)aw_kick_at(&workers[0], ++sequence,
                     timer_deadline() - timer_interval());
    for (unsigned j = 0; j < FRAMES * 2; ++j) pcm[j] = 0;
    got = 0;
  } else
#endif
    got = session_tick(&app, ++sequence, timer_deadline() - timer_interval(), pcm);
  if (got) {
    __atomic_fetch_add(&fresh, 1, __ATOMIC_RELAXED);
#ifdef SESSION_M11
    if (capture_mode && captured < 64) {
      for (unsigned j = 0; j < FRAMES * 2; ++j) {
        if (capture_mode == 1) reference_pcm[captured][j] = pcm[j];
        else if (reference_pcm[captured][j] != pcm[j]) ++wrong_samples;
      }
      ++captured;
    }
#endif
...
#ifdef SESSION_M11
  if (window_end && callbacks == window_end) timer_stop();
#endif
