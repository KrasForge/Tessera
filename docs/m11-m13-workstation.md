...
The dedicated M11 gate covers multicore scheduling/capacity/fault containment.
The dedicated M13 gate covers shell fuzzing, real-UART reporter interleaving,
console graph construction, console patch persistence, session codecs/live
swaps, and the production serial two-boot FAT/PCM acceptance. The combined gate
retains the shared temporal/session regressions used by both milestones.
...
The automated M13 serial correctness/cold-boot gate builds an explicit 12 kHz /
240-sample (50 Hz, 20 ms) emulation profile, keeping exact zero-missing-frame,
zero-skip, zero-plugin-offence and watchdog assertions. This isolates serial,
transactional-session and persistence correctness from host descheduling.
`test-arm-session-console-fast` retains the strict 48 kHz / 64-sample
wall-clock diagnostic. The functional profile is not a hardware latency/WCET
claim or a reclassification of earlier faster-profile failures.
...
The M13 functional profile explicitly uses 2,000 us budgets and 17,000/19,500 us
deadlines in its 20 ms frame. The wider dispatch slack prevents host vCPU
scheduling from masquerading as a shell failure while still requiring zero
plugin budget/deadline failures, zero missing/skipped output blocks and zero
watchdog overruns during live edits and the restored run. The fast diagnostic
retains its separate 400 us, 1,000/1,200 us contracts for the 1.333 ms frame.
The profile and counters are saved in each result JSON.
