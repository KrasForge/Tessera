...
class Console:
    ...
    def send(self, line: str, failure: bool=False) -> str:
        ...
        lines=[row.strip() for row in result.replace('\r','').split('\n') if row.strip()]
        errors=[row for row in lines if row.startswith('error:')]
        if failure:
            check(len(errors)==1 and len(lines)==2 and lines[-1]==errors[0],
                  f'failure must be input echo + exactly one error line {line!r}: {result}')
        else:
            check(not errors,f'unexpected command error {line!r}: {result}')
        return result
...
def main() -> None:
    ...
    try:
        ...
        for verb in ['load','unload','wire','unwire','set-param','ls','stats','patch','start','pause']:
            check(verb in help_text,'help lists '+verb)
        ...
        if frames==240 and rate==12000:
            # Functional 20 ms emulation period. The generous reservations keep
            # M13 independent of host descheduling; every miss is still fatal.
            c.send('contract 1 hard 2000us deadline=17000us policy=mute')
            c.send('contract 2 hard 2000us deadline=19500us policy=mute')
        elif frames==240 and rate==48000:
            # Retained 5 ms stress profile for explicit manual diagnostics.
        ...
        configured=c.send('ls')
        check('pid=1 /sd/SOURCE.ELF' in configured and 'pid=2 /sd/GAIN.ELF' in configured,
              'production ls shows plugin source paths')
        check('params: 0=0x3f000000' in configured,'production ls shows current parameter bits')
        check('1 -> 2' in configured and '2 -> dac' in configured,'production ls shows graph edges')
        ...
        c.send('pause');patches=c.send('patch ls')
        check('LIVE.TSP' in patches and '.ELF' not in patches,'patch ls lists sessions, not plugin ELFs')
    ...
    encoded_rate=(f'audio {rate} ').encode()
    bad_rate=44100 if rate!=44100 else 48000
    fat_add(image,'CLOCK.TSP',saved.replace(encoded_rate,(f'audio {bad_rate} ').encode()))
    ...
    try:
        ...
        check('params: 0=0x3e800000' in c.send('ls'),'ls reflects the live parameter update')
        stats=c.send('stats')
