...
int tc_host_bind(temporal_host_t *h, const tc_plugin_binding_t *b)
{
    ...
    __atomic_fetch_add(&b->call.plugin->host_refs, 1u, __ATOMIC_ACQ_REL);
...
int tc_host_detach(temporal_host_t *h)
{
    ...
    if (control_host == h) control_host = NULL;
    return TC_OK;
}

void tc_host_bind_control(temporal_host_t *h) { control_host = h; }
...
{
    if (!control_host) return TC_ENODEV;
    ...
    return tc_host_set(control_host, &c);
}
