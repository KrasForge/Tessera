...
/* Rounded IEEE-754 binary32 from an exact small decimal rational. Parsing
 * limits keep num < 2^62, so normalization and doubled remainders cannot
 * overflow. No floating-point instruction is used in the kernel. */
static uint32_t rational_bits(uint64_t num, uint64_t den) {
  if (!num)
    return 0;
  int exponent = 0;
  while (num >= den * 2) {
  ...
  if (rem * 2 > den || (rem * 2 == den && (mant & 1)))
    ++mant;
  if (mant == (1u << 24)) {
    mant >>= 1;
    ++exponent;
  }
  return ((uint32_t)(exponent + 127) << 23) | (mant & 0x7fffffu);
}
int patch_parse_value(const char *tok, uint32_t *bits) {
  ...
  unsigned i = 0, negative = 0, digits = 0, fraction_digits = 0;
  if (tok[i] == '-' || tok[i] == '+') {
    negative = tok[i] == '-';
    ++i;
  }
  uint64_t integer = 0, frac = 0, den = 1;
  while (tok[i] >= '0' && tok[i] <= '9') {
    unsigned d = (unsigned)(tok[i++] - '0');
    if (integer > (UINT32_MAX - d) / 10u)
      return PATCH_EFMT;
  ...
  if (!digits || tok[i])
    return PATCH_EFMT;
  uint32_t v = rational_bits(integer * den + frac, den);
  if (negative)
    v |= 0x80000000u;
  *bits = v;
  return PATCH_OK;
...
int patch_add_edge(patch_t *p, int src, int dst) {
  if (p->n_edges >= PATCH_MAX_EDGES)
    return PATCH_ERANGE;
  if (src != PATCH_INPUT && (src < 0 || src >= p->n_plugins))
    return PATCH_ERANGE;
  if (dst != PATCH_DAC && (dst < 0 || dst >= p->n_plugins))
    return PATCH_ERANGE;
...
long patch_serialize(const patch_t *p, char *out, uint32_t max) {
  ...
  for (int i = 0; i < p->n_plugins; i++)
    if (put(out, &pos, max, "plugin ") ||
        put(out, &pos, max, p->plugins[i].path) || put(out, &pos, max, "\n"))
      return PATCH_ENOSPACE;
  ...
  for (int i = 0; i < p->n_params; i++) {
    ...
    if (put(out, &pos, max, "param ") ||
        ...
        put(out, &pos, max, " ") || put_uint(out, &pos, max, p->params[i].id) ||
        put(out, &pos, max, " ") || put(out, &pos, max, val) ||
        put(out, &pos, max, "\n"))
      return PATCH_ENOSPACE;
  }
...
int patch_parse(const char *in, uint32_t len, patch_t *p) {
  ...
  while (i < len) {
    ...
    if (streq(kw, "plugin")) {
      if (token(in, &i, len, a, sizeof(a)) == 0)
        return PATCH_ETRUNC;
      if (patch_add_plugin(p, a) < 0)
        return PATCH_ERANGE;
    } else if (streq(kw, "param")) {
      if (token(in, &i, len, a, sizeof(a)) == 0)
        return PATCH_ETRUNC;
      if (token(in, &i, len, b, sizeof(b)) == 0)
        return PATCH_ETRUNC;
      if (token(in, &i, len, c, sizeof(c)) == 0)
        return PATCH_ETRUNC;
      uint32_t pi, id, bits;
      if (parse_uint(a, &pi) != PATCH_OK)
        return PATCH_EFMT;
      if (parse_uint(b, &id) != PATCH_OK)
        return PATCH_EFMT;
      if (patch_parse_value(c, &bits) != PATCH_OK)
        return PATCH_EFMT;
      if (patch_add_param(p, (int)pi, id, bits) < 0)
        return PATCH_ERANGE;
      ...
      if (streq(b, "dac")) {
        ...
        uint32_t d;
        if (parse_uint(b, &d) != PATCH_OK)
          return PATCH_EFMT;
        di = (int)d;
      }
      if (patch_add_edge(p, si, di) < 0)
        return PATCH_ERANGE;
    } else {
