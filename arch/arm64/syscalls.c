...
void arm64_handle_svc(struct trapframe *tf)
{
    ...
    if (gp && gp->svc_gate) {
        ...
        if (num != SYS_EXIT || elr < gp->svc_gate || elr >= gp->svc_gate + SVC_GATE_PAGE) {
            ...
            process_record_fault(gp, el0_cpu_index(), tf->esr_el1,
                                 tf->elr_el1, tf->far_el1, tf->spsr_el1);
...
{
    /* Retain the first fault even when the budgeted path suppresses UART.
     * Control can report full diagnostics after containment. The MMU has
     * already blocked the offending access; unwind with -1. */
    ...
    process_record_fault(p, el0_cpu_index(), tf->esr_el1, tf->elr_el1,
                         tf->far_el1, tf->spsr_el1);
    terminate_process(p);
