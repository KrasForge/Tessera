/* Frame-synchronous temporal contracts for each dedicated EL0 audio worker.
...
/* Structural contract/dependency validation and priority topological order,
 * without single-core capacity admission. Output remains unchanged on error. */
int tc_plan_validate(const audio_graph_t *graph, const temporal_contract_t *contracts,
                    uint32_t count, const tc_limits_t *limits,
                    tc_plan_t *out, tc_admission_t *why);
...
int tc_run_frame(temporal_scheduler_t *s, uint64_t frame, uint64_t release,
                 const tc_ops_t *ops);
/* Kernel multicore adapter: blocked bit i forbids executing task i and
 * publishes silence, with release/miss accounting but no budget offence. */
