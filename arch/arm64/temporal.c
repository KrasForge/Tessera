...
static int fail(tc_admission_t *why, int code, uint32_t pid, uint64_t required,
                uint64_t available) {
  if (why)
    *why = (tc_admission_t){code, pid, required, available};
  return code;
}
...
int tc_plan_validate(const audio_graph_t *g, const temporal_contract_t *c,
                     ...
                     tc_admission_t *why) {
  if (!g || !l || !out || (n && !c) || n > TC_MAX_TASKS || !l->frame_ticks ||
      l->frame_ticks > TC_TIME_MAX || l->frame_overhead >= l->frame_ticks ||
  ...
  for (int i = 0; i < GRAPH_MAX_NODES; ++i)
    if (g->nodes[i].type == NODE_PLUGIN)
      ++plugins;
  if (plugins != n)
    return fail(why, TC_ENODEV, 0, n, plugins);
  for (uint32_t i = 0; i < n; ++i) {
    ...
    for (uint32_t j = 0; j < i; ++j)
      if (c[j].pid == t->pid)
        return fail(why, TC_EINVAL, t->pid, 0, 0);
    if (!t->period || t->period > TC_TIME_MAX || t->period % l->frame_ticks ||
        ...
        (t->overrun_policy == TC_MUTE_THEN_KILL ? !t->kill_after
                                                : t->kill_after != 0) ||
        (t->overrun_policy == TC_DEGRADE
             ? (!t->skip_periods || t->skip_periods > 1024 ||
                t->criticality == TC_HARD)
             : t->skip_periods != 0))
  ...
  for (uint32_t k = 0; k < n; ++k) {
    ...
    for (uint32_t i = 0; i < n; ++i)
      if (!(done & (1u << i)) && !(p.deps[i] & ~done) &&
          (best < 0 || before(&p.task[i], &p.task[best])))
        best = (int)i;
    if (best < 0)
      return fail(why, TC_EDEPENDENCY, 0, 0, 0);
...
static void adopt(temporal_scheduler_t *s, uint64_t frame) {
  ...
  uint32_t old_count = s->active.count;
  for (uint32_t i = 0; i < old_count; ++i)
    old[i] = s->state[i];
  /* Copy live entries only. A singleton worker must not copy/clear sixteen
   * contracts and sixteen histories on every frame-boundary update. */
  s->active.limits = s->pending.limits;
  s->active.reserved_ticks = s->pending.reserved_ticks;
  s->active.count = s->pending.count;
  for (uint32_t i = 0; i < s->active.count; ++i) {
    s->active.task[i] = s->pending.task[i];
    s->active.deps[i] = s->pending.deps[i];
    s->active.order[i] = s->pending.order[i];
    ...
    for (uint32_t j = 0; j < old_count; ++j)
      if (old[j].pid == s->state[i].pid) {
        s->state[i] = old[j];
        break;
      }
  }
  for (uint32_t i = s->active.count; i < old_count; ++i)
    s->state[i] = (tc_task_state_t){0};
  s->active_valid = 1;
...
static void emit(const tc_ops_t *ops, tc_task_state_t *v, enum tc_output a) {
  if (a == TC_OUTPUT_SILENCE)
    add(&v->muted, 1);
  if (a == TC_OUTPUT_BYPASS)
    add(&v->bypassed, 1);
...
static uint64_t due_between(uint64_t base, uint64_t a, uint64_t b,
                            uint64_t period) {
  if (a > b || b < base)
    return 0;
...
int tc_run_frame_masked(temporal_scheduler_t *s, uint64_t frame,
                        uint64_t release, const tc_ops_t *ops,
                        uint32_t blocked) {
  if (!s || !ops || !ops->clock || !ops->run || !ops->output || !ops->kill ||
      !frame)
    return TC_EINVAL;
  if (s->started && frame <= s->last_frame)
    return TC_ESTALE;
  ...
  if (!ticks)
    return TC_ENODEV;
  if (release > UINT64_MAX - ticks || ops->clock(ops->ctx) < release)
    return TC_EINVAL;
  if (s->started) {
    uint64_t delta = frame - s->last_frame;
    if (delta > UINT64_MAX / ticks || release < s->last_release ||
        release - s->last_release != delta * ticks)
      return TC_EINVAL;
  }
  ...
  if (s->started) {
    for (uint32_t i = 0; i < s->active.count; ++i) {
      ...
      if (from < v->activated_frame)
        from = v->activated_frame;
  ...
  for (uint32_t k = 0; k < p->count; ++k) {
    ...
    if (cutoff > frame_end - tail)
      cutoff = frame_end - tail;
    ...
    if (elapsed > v->service_max)
      v->service_max = elapsed;
    if (end < now)
      result = -1; /* broken clock fails closed */
    if (result == TC_RUN_DEADLINE || end > deadline)
      add(&v->deadline_misses, 1);
    if (result < 0 && result != TC_RUN_BUDGET && result != TC_RUN_DEADLINE) {
      ...
      if (v->streak < UINT32_MAX)
        ++v->streak;
      if (t->overrun_policy == TC_KILL ||
          (t->overrun_policy == TC_MUTE_THEN_KILL &&
           v->streak >= t->kill_after))
        v->killed = 1;
