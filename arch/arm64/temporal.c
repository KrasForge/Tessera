                  ...
                  tc_admission_t *why)
{
    ...
    /* All jobs can release together. List-schedule that worst frame with
     * criticality-first, ready-job EDF, respecting same-frame precedence.
     * Any later release set is dependency-closed (producer period divides
     * consumer period); deleting absent jobs cannot make this schedule later.
     * Reserve fallback/publication time even for best-effort and non-due jobs. */
    for (uint32_t k = 0; k < n; ++k) {
        ...
        temporal_contract_t *t = &p.task[best];
