...
{
    if (!r || !__atomic_load_n(&r->initialized, __ATOMIC_ACQUIRE)) return TC_EINVAL;
    uint32_t idle = 0;
    if (!__atomic_compare_exchange_n(&r->editing, &idle, el0_cpu_index() + 1u, 0,
                                     __ATOMIC_ACQUIRE, __ATOMIC_RELAXED)) return TC_EBUSY;
    if (!__atomic_load_n(&r->initialized, __ATOMIC_ACQUIRE)) {
...
static int may_mutate(void *ctx)
{
    ...
    return __atomic_load_n(&r->editing, __ATOMIC_ACQUIRE) == el0_cpu_index() + 1u &&
        !r->running && aw_paused(r->worker);
}

static int control_set(void *ctx, const temporal_contract_t *contract)
{ return tr_set_contract(ctx, contract); }
             ...
             const tr_io_ops_t *io)
{
    ...
    *r = (temporal_runtime_t){0};
    ...
    r->host.control_set = control_set; r->host.control_ctx = r;
    int rc = tc_host_attach(&r->host, w);
    if (rc != TC_OK) return rc;
    aw_pause(w);
    pm_set_lifecycle_budget(m, lifecycle_ticks);
    __atomic_store_n(&r->initialized, 1u, __ATOMIC_RELEASE);
...
int tr_set_contract(temporal_runtime_t *r, const temporal_contract_t *contract)
{
    int rc = enter(r, 0);
    if (rc != TC_OK) return rc;
    if (!contract) { leave(r); return TC_EINVAL; }
    if (r->running) rc = tc_host_set(&r->host, contract);
    else {
        temporal_contract_t next[TC_MAX_TASKS];
        int found = 0;
        for (uint32_t i = 0; i < r->host.desired_count; ++i) {
            next[i] = r->host.desired[i];
            if (next[i].pid == contract->pid) { next[i] = *contract; found = 1; }
        }
        tc_plan_t plan;
        rc = found ? tc_plan_build(&r->manager->gc->graph, next, r->host.desired_count,
            &r->host.limits, &plan, &r->host.last_admission) : TC_ENODEV;
        if (rc == TC_OK) {
            for (uint32_t i = 0; i < r->host.desired_count; ++i) r->host.desired[i] = next[i];
            r->host.scheduler.active_valid = 0;
        }
    }
    leave(r);
...
int tr_shutdown(temporal_runtime_t *r, uint64_t timeout)
{
    ...
    if (rc == TC_OK) {
        ...
        __atomic_store_n(&r->initialized, 0u, __ATOMIC_RELEASE);
    }
    leave(r);
    return rc;
}
