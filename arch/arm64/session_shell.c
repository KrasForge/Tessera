...
static int report(shell_t *sh, int rc) {
  ...
  if (rc == TC_EINVAL) {
    shell_write(sh, "invalid argument or unsupported configuration");
  } else if (rc == TC_EADMISSION) {
  ...
  else if (rc == SESSION_EIO)
    shell_write(sh, "file not found or storage I/O error");
  else if (rc == SESSION_EFULL)
    shell_write(sh, "session capacity exhausted");
  else if (rc == SESSION_ETIMEOUT)
    shell_write(sh, "control transaction timed out");
  else if (rc == SESSION_EINPROGRESS)
    shell_write(sh, "boundary swap accepted; completion pending");
  else if (rc == TC_EBUSY)
    ...
    if (rc < 0) {
      shell_write(sh, "-");
      number(sh, (uint64_t)(-(int64_t)rc));
    } else
      number(sh, (uint64_t)rc);
  }
  shell_write(sh, "\r\n");
  /* The command has already rendered its single user-visible error line.
   * Return success to the generic dispatcher so it cannot append a second,
   * numeric error line. The underlying operation has already failed and is
   * transactional; no state is committed by this reporting choice. */
  return 0;
}
  ...
  for (unsigned i = 0; i < sc->count; ++i) {
    ...
    for (unsigned j = 0; j < PM_MAX_PLUGINS; ++j)
      if (sc->nodes[i]->manager->slots[j].used &&
        ...
        pm = &sc->nodes[i]->manager->slots[j];
        shell_write(sh, pm->path);
        break;
      }
    if (pm && pm->n_params) {
      shell_write(sh, " params:");
      for (int k = 0; k < pm->n_params; ++k) {
        char bits[11];
        shell_write(sh, " ");
        number(sh, pm->params[k].id);
        shell_write(sh, "=");
        patch_format_value(pm->params[k].bits, bits);
        shell_write(sh, bits);
      }
    }
    shell_write(sh, " cpu=");
...
static int stats(shell_t *sh, int argc, char **v) {
  ...
  for (unsigned i = 0; i < sc->count; ++i) {
    ...
    if (process_fault_snapshot(sc->nodes[i]->plugin->proc, &fault)) {
      field(sh, " fault_cpu=", fault.cpu);
      field(sh, " esr=", fault.esr);
      field(sh, " elr=", fault.elr);
      field(sh, " far=", fault.far);
      field(sh, " spsr=", fault.spsr);
    }
    shell_write(sh, "\n");
  }
  return 0;
...
static int is_session_file(const char *name) {
  unsigned n = 0;
  while (name[n])
    ++n;
  if (n < 4 || name[n - 4] != '.')
...
static void filename(void *ctx, const char *name) {
  patch_list_ctx_t *list = ctx;
  if (!is_session_file(name))
    return;
  shell_write(list->sh, "  ");
  shell_write(list->sh, name);
  shell_write(list->sh, "\r\n");
  ++list->count;
}
static int patch(shell_t *sh, int argc, char **v) {
  ...
  if (eq(v[1], "ls")) {
    ...
    patch_list_ctx_t list = {sh, 0};
    shell_write(sh, "patches:\r\n");
    for (unsigned i = 0; i < VFS_STORE_SLOTS; ++i)
      if (fs->store[i].used)
        filename(&list, fs->store[i].name);
    if (fs->fat && fat_list(fs->fat, filename, &list) < 0)
      return report(sh, SESSION_EIO);
    if (!list.count)
      shell_write(sh, "  (none)\r\n");
    return 0;
  }
